...
import com.monogatari.app.profile.domain.use_cases.GetProfileInfoLocalUseCase
...
class ProfileViewModel(app : Application): AndroidViewModel(app) {
    ...
    private val _getProfileInfoUseCase = GetProfileInfoUseCase(app)
    private val _getProfileInfoLocalUseCase = GetProfileInfoLocalUseCase(app)
    ...
    private fun getInfoLocal() {
        viewModelScope.launch {
            try {
                state.value = LibraryState.Loading
                val result = _getProfileInfoLocalUseCase.execute()
                state.value = LibraryState.Success(result)
                Log.d("PROFILE_VM_TAG", "getInfo: $result")
            } catch (e: Exception) {
